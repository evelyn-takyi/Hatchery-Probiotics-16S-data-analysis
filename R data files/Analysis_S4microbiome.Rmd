...
## Count table
abund_table <- read.xlsx("~/Downloads/Data.xlsx", sheet="table", rowNames = TRUE)

## Metadata table
meta_table <-read.xlsx("~/Downloads/Data.xlsx", sheet="meta_table", rowNames = TRUE)

## Taxonomy table
taxa_table <- read_excel("~/Downloads/Data.xlsx", sheet = "taxonomy") %>% 
...
Order_abundances <- justbacteria_12000_rarefy %>% tax_glom(taxrank = "Order") %>% 
  ...
  Order_abundances
  write.csv( Order_abundances, "order.csv")
...
write.csv( Average, "diversity.csv")
##Simpson's diversity
...
#Calculate p-value:
adon_NUV_UV<-adonis2(ASV_NUV_UV ~ Condition, data=Meta_NUV_UV, by=NULL,method="bray", k=2)
...
#rarecurveraredata <- map_dfr(rarecurve_data, bind_rows) %>% 
 #  bind_cols(SampleID = rownames(t(otu_table(justbacteria_12000_rarefy))),.) %>%
  ...
  # scale_x_continuous(labels=scales::label_comma())+
   #scale_y_continuous(limits=c(0,400))+
   #theme(legend.key.width = unit(1.33,"cm"), legend.position="right")+
   #3labs(x = "Number of sequences", y="Number of ASVs detected", color=NULL, lty=NULL, #title="Rarefied data")
#rarecurveraredata
...
Phylum_abundance 
write.csv(Phylum_abundance,"Phylum.csv")
...
div_Vib1
write.csv(div_Vib1,"Vibrio_diversity.csv")
