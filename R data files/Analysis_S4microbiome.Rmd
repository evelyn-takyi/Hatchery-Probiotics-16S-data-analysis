...
justbacteria_12000_rarefy %>% tax_glom(taxrank = "Order") %>% 
   psmelt() %>% 
...
Div <- adiv %>% group_by(Num,Trial,Treatment,Tank_Replicate) %>% summarise(mean_read=mean(Simpson), sd_read=sd(Simpson))

# plotting
Div_SIM <- ggplot(Div, aes(x=Treatment, y=mean_read, fill=Treatment))+
...
Simpsons_Div = lmer(mean_Simpsons ~ Trial*Treatment+(1|Tank_Replicate), data=Average, REML = FALSE)
...
emm.div = emmeans(Simpsons_Div, ~ Treatment | Trial)
...
```{r betasummary, fig.width=10, height=10}
...
Permanova <-adonis2(BC.dist ~ Hatchery*Location*Treatment*Trial, data = Meta2, permutations = 1000)
Permanova
...
Div_UV <- ggplot(DIVS, aes(as.factor(Condition), mean_div, fill=Treatment))+
  ...
  geom_signif(comparisons=list(c("C_NUV", "C_UV")), annotations="**", 
   ...
   geom_signif(comparisons=list(c("C_UV", "S4_UV")), annotations="**", y_position = 0.95, 
               tip_length = 0.1)+
...
## Statistics
## Run the model
Simpsons_Div = lme4::lmer(mean_div ~ Type*Treatment+(1|Tank_Replicate), data=DIVS, REML = FALSE)
summary(Simpsons_Div)
## Run ANOVA on the model to determine statistical significance of the factors
anova(Simpsons_Div)
emm.trajectory = emmeans::emmeans(Simpsons_Div, ~ Treatment | Type)
Diversity_Comp = data.frame(emmeans::contrast(emm.trajectory, interaction = "pairwise", adjust = "Bonferroni"))
Diversity_Comp
...
ASV_NUV <- subset_samples(justbacteria_12000_rarefy, UV_NUV == "Y") %>% psmelt() %>%
   group_by(Num,OTU) %>% summarise(avg_abundance = mean(Abundance)) %>%  # average by tank
   spread(Num, avg_abundance) %>% column_to_rownames(var="OTU") %>% t() %>% as.data.frame()
Meta_NUV <- All_Meta[rownames(ASV_NUV),] %>% 
   mutate(Type=word(Condition, 2, sep = "_"))

sol_NUV <- metaMDS(ASV_NUV, distance = "bray", k = 2)
NMDS_NUV=data.frame(NMDS1=sol_NUV$point[,1], NMDS2=sol_NUV$point[,2], 
                    Condition=as.factor(Meta_NUV$Condition),
                    Treatment=as.factor(Meta_NUV$Treatment),
                    Type=as.factor(Meta_NUV$Type))

plot.new()
ord_NUV<-ordiellipse(sol_NUV, as.factor(Meta_NUV$Condition), display = "sites", kind ="sd", conf = 0.95, label = T)
dev.off()

df_ell_NUV <- data.frame()
for(g in levels(NMDS_NUV$Condition)){
  if(g!="" && (g %in% names(ord_NUV))){
    df_ell_NUV <- rbind(df_ell_NUV, cbind(as.data.frame(with(NMDS_NUV[NMDS_NUV$Condition==g,],                            veganCovEllipse(ord_NUV[[g]]$cov,ord_NUV[[g]]$center,ord_NUV[[g]]$scale))),Condition=g))}}

#Calculate p-value:
adon_NUV<-adonis2(ASV_NUV ~Condition, data=Meta_NUV, by=NULL,method="bray", k=2)

#Plot NMDS
NMDSplot_NUV<-ggplot(data=NMDS_NUV,aes(NMDS1,NMDS2))+
  annotate("text",x=min(NMDS_NUV$NMDS1-0.5),y=min(NMDS_NUV$NMDS2-0.3), 
           label=paste("p-value = ", round(adon_NUV$`Pr(>F)`[1],3)), hjust=0)+
  annotate("text",x=min(NMDS_NUV$NMDS1-0.5),y=min(NMDS_NUV$NMDS2-0.22), 
           label=paste("Stress = ", round(sol_NUV$stress,2)), hjust=0)+
...
## Statistics
##Permanova
BC.dist<- vegdist(Abun, method="bray")
#Permanova <-adonis2(BC.dist ~ Type*Treatment, data = Meta, permutations = 1000)
#Permanova
...
```{r figure6, fig.width=10}
## Plot together

Div_UV + NMDSplot_NUV + plot_annotation(tag_levels = "A")
ggsave("Figures/Figure6.png", width=12, height=5, dpi=400)
...
justbacteria_12000_rarefy %>% tax_glom(taxrank = "Phylum") %>% 
   psmelt() %>% 
...
adiv <- data.frame(
  ...
  "Shannon" = phyloseq::estimate_richness(phy, measures = "Shannon"),
  "Simpson" = phyloseq::estimate_richness(phy, measures = "Simpson"),
  "Chao1"= phyloseq::estimate_richness(phy, measures = "Chao1"),
  "Status" = phyloseq::sample_data(phy)$Treatment)
adiv$Trial <- SD$Trial
...
NMDS_all=data.frame(NMDS1=sol_all$point[,1], NMDS2=sol_all$point[,2], 
                   ...
                   Hatchery=as.factor(Meta_all$Hatchery),
                   Trial=as.factor(Meta_all$Trial))

plot.new()
ord_hatchery<-ordiellipse(sol_all, as.factor(Meta_all$Hatchery), display = "sites", kind ="sd", conf = 0.95, label = T)
dev.off()

df_ell_hatchery <- data.frame()
for(g in levels(NMDS_all$Hatchery)){
  if(g!="" && (g %in% names(ord_hatchery))){
    df_ell_hatchery <- rbind(df_ell_hatchery, cbind(as.data.frame(with(NMDS_all[NMDS_all$Hatchery==g,],                            veganCovEllipse(ord_hatchery[[g]]$cov,ord_hatchery[[g]]$center,ord_hatchery[[g]]$scale))),Hatchery=g))}}

#Calculate p-value:
adon_hatchery<-adonis2(ASV_all ~Hatchery, data=Meta_all, by=NULL,method="bray", k=2)

#Plot NMDS
NMDSplot_hatchery<-ggplot(data=NMDS_all,aes(NMDS1,NMDS2,col=Hatchery))+
  annotate("text",x=-4,y=3.5, 
           label=paste("p-value = ", round(adon_hatchery$`Pr(>F)`[1],3)), hjust=0)+
  annotate("text",x=-4,y=3.1, 
          label=paste("Stress = ", round(sol_all$stress,2)), hjust=0)+
  geom_path(data=df_ell_hatchery, aes(x=NMDS1, y=NMDS2, linetype=Hatchery), size=1)+
  scale_linetype_manual(values=c("VIMS"="solid","MK"="dotted","RWU"="dashed"))+
  scale_colour_manual(values=c("VIMS"="orange","MK"="brown","RWU"="tomato"))+
  ggtitle("Hatchery")+
  geom_point(aes(shape=Treatment), size=3, alpha=.8) + scale_shape_manual(values=c(15,16)) + 
   theme_grey()+
   theme(plot.title=element_text(size = 16, face = "bold", hjust = 0.5))
...
plot.new()
ord_treatment<-ordiellipse(sol_all, as.factor(Meta_all$Treatment), display = "sites", kind ="sd", conf = 0.95, label = T)
dev.off()

df_ell_treatment <- data.frame()
for(g in levels(NMDS_all$Treatment)){
  if(g!="" && (g %in% names(ord_treatment))){
    df_ell_treatment <- rbind(df_ell_treatment, cbind(as.data.frame(with(NMDS_all[NMDS_all$Treatment==g,],                            veganCovEllipse(ord_treatment[[g]]$cov,ord_treatment[[g]]$center,ord_treatment[[g]]$scale))),Treatment=g))}}

#Calculate p-value:
adon_treatment<-adonis2(ASV_all ~Treatment, data=Meta_all, by=NULL,method="bray", k=2)

#Plot NMDS
NMDSplot_treatment<-ggplot(data=NMDS_all,aes(NMDS1,NMDS2,col=Treatment))+
  annotate("text",x=-4,y=3.5, 
           label=paste("p-value = ", round(adon_treatment$`Pr(>F)`[1],3)), hjust=0)+
  annotate("text",x=-4,y=3.1, 
          label=paste("Stress = ", round(sol_all$stress,2)), hjust=0)+
  geom_path(data=df_ell_treatment, aes(x=NMDS1, y=NMDS2, linetype=Treatment), size=1)+
...
```{r plotfigs4, fig.height=15}

NMDSplot_hatchery / NMDSplot_trial / NMDSplot_treatment +
   plot_annotation(tag_levels = "A") &
   theme(legend.position = "right", legend.direction = "vertical", legend.justification = c(0.5,1))

ggsave("Figures/FigureS4.png",width=6, height=10, dpi = 400)
...
## Run the model
Simpsons_Div = lme4::lmer(mean_div ~ Trial*Treatment+(1|Tank_Replicate), data=div_Vib1, REML = FALSE)
summary(Simpsons_Div)
## Run ANOVA on the model to determine statistical significance of the factors
Anova(Simpsons_Div)
emm.trajectory = emmeans::emmeans(Simpsons_Div, ~ Treatment | Trial)
DivVibrio_Comp = data.frame(emmeans::contrast(emm.trajectory, interaction = "pairwise", adjust = "Bonferroni"))
DivVibrio_Comp
...
##Statistics
## Run the model
Vib_Abun = lme4::lmer(VibrioAbundance ~ Trial*Treatment+(1|Tank_Replicate), data=vibrioabundances, REML = FALSE)
summary(Vib_Abun)
